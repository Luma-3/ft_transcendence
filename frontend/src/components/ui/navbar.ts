...
export async function navbar(user: IUserInfo) {
	...
	//TODO: Traduction
	return `
	<nav id="navbar" class="flex flex-row navbar-responsive-size items-center w-full mt-2 flex-wrap justify-between p-2">
			${navbarLogo()}
			...
			<span class="dot-anim"> Recherche d'une partie en cours...</span>
			</div>
		<button id="cancel-waiting-game" class="px-2 py-1 text-sm rounded bg-dprimary text-white shadow hover:bg-dsecondary transition-colors duration-200">
				Annuler
			</button>
			</div>` : ""}
